...
const securityDomains = [
  {
    ...
    description: "wgtwo performs threat modeling across its telco services to understand the threats/risks and implement countermeasures to reduce the risk of an event. Additionally, wgtwo leverages best practices identified by GSMA to implement additional controls.",
  },
    ...
    <div className={styles.securityDomains}>
      {securityDomains.slice(0, props.length).map((securityDomain) => (
        <div className={styles.securityDomain} key={securityDomain.name}>
          ...
          <a href={`/docs/security-whitepaper#${securityDomain.name.replaceAll(" ", "-").toLowerCase()}`} className={styles.link}>
            Learn more
          </a>
